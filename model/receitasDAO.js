class receitasDAO {
  ...
  editeReceita(receita, callback) {
    ...
    this.db.get(
      ...
      (err, row) => {
        this.db.run(
            `UPDATE receitas SET titulo = ?, url = ?, image = ?, ingredientes = ?, preparo = ?, categoryID = ?, local = ?, nivel = ?, autor = ?`,
            [titulo, url, image, ingredientes, preparo, categoryID, local, nível, autor],
            callback
        );
      }
    );
